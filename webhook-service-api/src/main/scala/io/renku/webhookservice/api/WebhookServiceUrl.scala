...
object WebhookServiceUrl
  ...
  type T = WebhookServiceUrl

  implicit val webhookServiceUrlOps: WebhookServiceUrl.type = this

  private implicit val urlReader: ConfigReader[WebhookServiceUrl] = urlTinyTypeReader(WebhookServiceUrl)

  def apply[F[_]: MonadThrow](config: Config = ConfigFactory.load): F[WebhookServiceUrl] =
