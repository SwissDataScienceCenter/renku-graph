...
class CleanUpEventsQueueSpec
  ...
  "offer" should {
    ...
    "insert a new row if there's no row for the project" in new TestCase {
      ...
      queue.offer(path1).unsafeRunSync() shouldBe ()
      queue.offer(path2).unsafeRunSync() shouldBe ()

      findCleanUpEvents shouldBe List(path1, path2)
    }

    "do not insert duplicates" in new TestCase {
      val path = projectPaths.generateOne

      queue.offer(path).unsafeRunSync() shouldBe ()
      queue.offer(path).unsafeRunSync() shouldBe ()

      findCleanUpEvents shouldBe List(path)
    }
  }
