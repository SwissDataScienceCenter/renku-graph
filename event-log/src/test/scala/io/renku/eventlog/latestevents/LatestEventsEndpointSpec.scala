...
class LatestEventsEndpointSpec extends AnyWordSpec with MockFactory with should.Matchers {

  "findLatestEvents" should {

    "return a list of event id, project and body found in the Log" in new TestCase {

      val idProjectBodyList = nonEmptyList(newOrSkippedEvents).generateOne.toList.map(toIdProjectBody)
