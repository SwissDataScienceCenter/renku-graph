    ...
    "factory" should {
      ...
      "return a CommandFound when the change status request is acceptable" in new TestCase {
        ...
        val actual = ToGenerationNonRecoverableFailure
          ...
          .run(EventOnlyRequest(eventId, GenerationNonRecoverableFailure, maybeProcessingTime, Some(message)))
      ...
      "return a PayloadMalformed when status change request does not have a message" in new TestCase {
        ToGenerationNonRecoverableFailure
          .factory[IO](underTriplesGenerationGauge)
          .run(EventOnlyRequest(eventId, GenerationNonRecoverableFailure, eventProcessingTimes.generateOption, None))
