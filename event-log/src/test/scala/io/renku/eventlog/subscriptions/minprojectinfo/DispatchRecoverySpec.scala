...
class DispatchRecoverySpec extends AnyWordSpec with IOSpec with InMemoryEventLogDbSpec with should.Matchers {
  ...
  "returnToQueue" should {
    ...
    s"delete the row from the subscription_category_sync_times table for the $categoryName and project" in new TestCase {
      ...
      recovery
        .returnToQueue(MinProjectInfoEvent(event.projectId, event.projectPath), sendingResults.generateOne)
  ...
  "recover" should {
    ...
    s"delete the row from the subscription_category_sync_times table for the $categoryName and project" in new TestCase {
      ...
      recovery
        .recover(subscriber, MinProjectInfoEvent(event.projectId, event.projectPath))(exception)
        .unsafeRunSync() shouldBe ()
