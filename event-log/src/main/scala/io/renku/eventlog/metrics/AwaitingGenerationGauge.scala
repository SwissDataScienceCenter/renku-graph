...
trait AwaitingGenerationGauge[F[_]] extends LabeledGauge[F, projects.Path]

object AwaitingGenerationGauge {

  val NumberOfProjects: Int Refined Positive = 20

  def apply[F[_]: MonadThrow: MetricsRegistry](statsFinder: StatsFinder[F]): F[AwaitingGenerationGauge[F]] =
    MetricsRegistry[F].register {
      new LabeledGaugeImpl[F, projects.Path](
        name = "events_awaiting_generation_count",
        help = "Number of Events waiting to have their triples generated by project path.",
        labelName = "project",
        resetDataFetch = () =>
          statsFinder
  ...
  def apply[F[_]](implicit ev: AwaitingGenerationGauge[F]): AwaitingGenerationGauge[F] = ev
}
