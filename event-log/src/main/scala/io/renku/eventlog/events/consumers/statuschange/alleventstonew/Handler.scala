...
package io.renku.eventlog.events.consumers.statuschange.alleventstonew
...
private[statuschange] object Handler {
  ...
  def apply[F[_]: Async: Logger: MetricsRegistry: QueriesExecutionTimes](
    ...
    (DbUpdater[F] -> ProcessExecutor.concurrent(5))
      .mapN((updater, executor) => new Handler[F](categoryName, updater, statusChanger, executor))
}
