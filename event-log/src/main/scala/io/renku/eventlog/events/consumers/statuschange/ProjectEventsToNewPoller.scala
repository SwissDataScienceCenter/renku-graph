...
private trait ProjectEventsToNewPoller[F[_]] extends DBUpdater[F, ProjectEventsToNew]

private object ProjectEventsToNewPoller {
  def apply[F[_]: Async: AccessTokenFinder: Logger: QueriesExecutionTimes]: F[DBUpdater[F, ProjectEventsToNew]] =
    ProjectCleaner[F].map(new ProjectEventsToNewPollerImpl(_))
}

private class ProjectEventsToNewPollerImpl[F[_]: Async: Logger: QueriesExecutionTimes](
    projectCleaner: ProjectCleaner[F],
    now:            () => Instant = () => Instant.now
) extends DbClient(Some(QueriesExecutionTimes[F]))
    with ProjectEventsToNewPoller[F] {
