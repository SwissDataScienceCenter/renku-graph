...
import ch.datascience.graph.acceptancetests.stubs.RemoteTriplesGenerator
...
class EventFlowsSpec
  ...
  Feature("Push events from GitLab should be translated into triples in the RDF Store") {
    ...
    Scenario("A recoverable transformation error arises and the events are reported as a recoverable failure") {
      ...
      RemoteTriplesGenerator.reset()

      implicit val accessToken: AccessToken = accessTokens.generateOne
      val project  = dataProjects(projectEntities(visibilityPublic)).generateOne
      val commitId = commitIds.generateOne

      Given("the Triples Store is down to make transformation to fail recoverably")
      RDFStore.stop().unsafeRunSync()
      ...
      eventually {
        EventLog.findEvents(project.id).map(_._2).toSet shouldBe Set(TransformationRecoverableFailure)
      }

      // Starting up the triples store for the next specs
      RDFStore.start().unsafeRunSync()
    }
  }
}
