...
class ELCommitFetcherSpec extends AnyWordSpec with IOSpec with ExternalServiceStubbing with should.Matchers {
  ...
  "fetchGitLabCommits" should {
    ...
    "return no commits if there aren't any" in new TestCase {
      ...
      stubFor {
        get(
          s"/events?project-path=$projectSlug&page=${pageRequest.page}&per_page=${pageRequest.perPage}&until=${urlEncode(dateCondition.date.toString)}"
        ).willReturn(okJson("[]"))
      }
    ...
    "return an Exception if remote client responds with status neither OK nor UNAUTHORIZED" in new TestCase {
      ...
      }.getMessage shouldBe s"GET $eventLogUrl/events?project-path=$projectSlug&page=${pageRequest.page}&per_page=${pageRequest.perPage}&until=${urlEncode(
    ...
    "return an Exception if remote client responds with unexpected body" in new TestCase {
      ...
      intercept[Exception] {
        ...
        s"GET $eventLogUrl/events?project-path=$projectSlug&page=${pageRequest.page}&per_page=${pageRequest.perPage}&until=${urlEncode(
            dateCondition.date.toString
