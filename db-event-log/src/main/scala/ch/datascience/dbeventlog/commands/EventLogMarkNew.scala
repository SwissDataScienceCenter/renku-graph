...
class EventLogMarkNew[Interpretation[_]](
    ...
    case _ =>
      ME.raiseError(
        new RuntimeException(
          s"Event with id = ${commitEventId.id}, projectId = ${commitEventId.projectId} couldn't be marked as $New; either no event or not with status $Processing"
        )
      )
  }
}

class IOEventLogMarkNew(
    transactor:          DbTransactor[IO, EventLogDB]
)(implicit contextShift: ContextShift[IO])
    extends EventLogMarkNew[IO](transactor)
