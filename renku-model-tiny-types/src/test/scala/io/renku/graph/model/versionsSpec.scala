...
class versionsSpec extends AnyWordSpec with ScalaCheckPropertyChecks with should.Matchers with RenkuTinyTypeGenerators {
  ...
  "ordering" should {
    ...
    "consider the dev part if all majors, minors and bugfix are the same" in {
      ...
      forAll(devVersions(Generators.fixed(semanticVersion)), devVersions(Generators.fixed(semanticVersion))) {
        (version1, version2) =>
  ...
  private def devVersions(semanticVersionsGen: Gen[String] = Generators.semanticVersions) =
    (semanticVersionsGen, Generators.positiveInts(999), Generators.shas.map(_.take(7)))
