...
class datasetsSpec extends AnyWordSpec with ScalaCheckPropertyChecks with should.Matchers with EitherValues {
  ...
  "SameAs.internal" should {
    ...
    "return an instance of InternalSameAs" in {
      ...
      val instance = SameAs.internal(RenkuUrl(sameAs.value)).value
  ...
  "SameAs.from" should {
    ...
    "return an instance of ExternalSameAs" in {
      val sameAs = datasetExternalSameAs.generateOne

      val instance = SameAs.from(sameAs.value).value
  ...
  "SameAs.of" should {
    ...
    "return an instance of InternalSameAs when the given URL is a Renku URL" in {

      val sameAs = SameAs.internal(renkuUrl / datasetIdentifiers.generateOne).value

      val instance = SameAs.of(sameAs.value).value

      instance       shouldBe an[InternalSameAs]
      instance.value shouldBe sameAs.value
    }

    "return an instance of ExternalSameAs when the given URL is not a Renku URL" in {
  ...
  "SameAs jsonLdEncoder" should {
    ...
    "serialise InternalSameAs to an object having url property linked to the SameAs's value" in {
      val sameAs = datasetInternalSameAs.generateOne
    ...
    "serialise ExternalSameAs to an object having url property as the SameAs's value" in {
      val sameAs = datasetExternalSameAs.generateOne
  ...
  "CreatedOrPublished" should {
    ...
    "provide decoder if it's a DateCreated" in {
      val date: DateCreated = datasetCreatedDates().generateOne
      date.asInstanceOf[CreatedOrPublished].asJson.hcursor.as[CreatedOrPublished].value shouldBe date
    }

    "provide decoder if it's a DatePublished" in {
      val date: DatePublished = datasetPublishedDates().generateOne
      date.asInstanceOf[CreatedOrPublished].asJson.hcursor.as[CreatedOrPublished].value shouldBe date
    }
  }
