...
class EventsDequeueingInitiatorSpec extends AsyncFlatSpec with AsyncIOSpec with should.Matchers with AsyncMockFactory {

  it should "wait with hooking the processor into the stream from the dequeuer until the TS state is not ready" in {
    ...
    for {
      ...
      _ <- EventsDequeueingInitiator[IO](tsReadyCheck,
                                         eventsQueue,
                                         processor,
                                         onTSNotReadyWait = 100 millis
           ).initDequeueing.assertNoException
      _   <- processor.handledEvents.waitUntil(_ == chunk)
      res <- tsReadyResponses.get.asserting(_.isEmpty shouldBe true)
    } yield res
  }

  it should "hook the processor into the stream acquired from the dequeuer" in {
    ...
    for {
      processor <- newAccumulatingHandler()
      _         <- EventsDequeueingInitiator[IO](IO(true), eventsQueue, processor).initDequeueing.assertNoException
      _         <- processor.handledEvents.waitUntil(_ == chunk1 ::: chunk2)
    } yield Succeeded
  }

  it should "handle failures in the dequeueing process so the process does not die" in {
    ...
    for {
      ...
      _ <- EventsDequeueingInitiator[IO](IO(true),
                                         eventsQueue,
                                         processor,
                                         onErrorWait = 100 millis
           ).initDequeueing.assertNoException
      _ <- IO.race(
  ...
  it should "handle failures happening while checking the TS state" in {
