...
trait TestSearchInfoDatasets {
  self: TestProjectsDataset =>

  implicit def ioLogger: Logger[IO]

  def provisionTestProjects(projects: testentities.Project*)(implicit
      renkuUrl:        RenkuUrl,
  ...
  def provisionTestProject(p: testentities.Project)(implicit
      renkuUrl:        RenkuUrl,
      entityFunctions: EntityFunctions[entities.Project],
      graphsProducer:  GraphsProducer[entities.Project],
      pcc:             ProjectsConnectionConfig
  ...
  def provisionProject(project: entities.Project)(implicit
      entityFunctions: EntityFunctions[entities.Project],
      graphsProducer:  GraphsProducer[entities.Project],
      renkuUrl:        RenkuUrl,
      pcc:             ProjectsConnectionConfig
  ): IO[Unit] =
    uploadToProjects(graphsProducer(project): _*) >> insertSearchInfo(project) >> insertProjectAuth(project)

  def insertProjectAuth(
      project: entities.Project
  )(implicit renkuUrl: RenkuUrl, pcc: ProjectsConnectionConfig): IO[Unit] = {
    ...
    val ps      = ProjectSparqlClient[IO](pcc).map(ProjectAuthService[IO](_, renkuUrl))
  ...
  def insertSearchInfo(project: entities.Project)(implicit pcc: ProjectsConnectionConfig): IO[Unit] =
    createSearchGraphsProvisioner.flatMap(_.provisionSearchGraphs(project))

  def createSearchGraphsProvisioner(implicit pcc: ProjectsConnectionConfig): IO[SearchGraphsProvisioner[IO]] = {
    val execTimeRecorder: ExecutionTimeRecorder[IO] = TestExecutionTimeRecorder[IO]()
    implicit val sparqlQueryTimeRecorder: SparqlQueryTimeRecorder[IO] =
      new SparqlQueryTimeRecorder[IO](execTimeRecorder)

    IO(SearchGraphsProvisioner[IO](Lock.none[IO, datasets.TopmostSameAs], pcc))
  }
}
