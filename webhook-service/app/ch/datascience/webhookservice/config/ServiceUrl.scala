...
object ServiceUrl extends TinyTypeFactory[URL, ServiceUrl]( new ServiceUrl( _ ) ) {
  ...
  implicit object FusekiUrlFinder extends ConfigLoader[ServiceUrl] {
    override def load( config: Config, path: String ): ServiceUrl = ServiceUrl( config.getString( path ) )
  }

  implicit class FusekiUrlOps( fusekiUrl: ServiceUrl ) {
    def /( value: Any ): ServiceUrl = ServiceUrl(
      new URL( s"$fusekiUrl/$value" )
    )
  }

  implicit def asString( fusekiUrl: ServiceUrl ): String = fusekiUrl.toString
}
