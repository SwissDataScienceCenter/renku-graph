...
private object IOHookCreator {
  def apply(
    ...
    for {
      eventsHistoryLoader <- IOEventsHistoryLoader(transactor,
                                                   tokenRepositoryUrl,
                                                   gitLabUrl,
                                                   gitLabThrottler,
      ...
      hookTokenCrypto,
      new IOProjectHookCreator(gitLabUrl, gitLabThrottler, logger),
      new IOAccessTokenAssociator(tokenRepositoryUrl, logger),
      eventsHistoryLoader,
      logger
    )
}
