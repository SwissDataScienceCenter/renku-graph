...
import io.renku.tinytypes.{IntTinyType, LongTinyType, TinyTypeFactory}
...
class NonNegativeSpec extends AnyWordSpec with ScalaCheckPropertyChecks with should.Matchers {
  ...
  "NonNegativeInt" should {
    ...
    "throw an IllegalArgumentException for negative value" in {
      ...
      ).getMessage shouldBe "io.renku.tinytypes.constraints.TestNonNegativeInt cannot be < 0"
    }
  }
  ...
  "NonNegativeLong" should {
    ...
    "throw an IllegalArgumentException for negative value" in {
      ...
      ).getMessage shouldBe "io.renku.tinytypes.constraints.TestNonNegativeLong cannot be < 0"
    }
  }
}
