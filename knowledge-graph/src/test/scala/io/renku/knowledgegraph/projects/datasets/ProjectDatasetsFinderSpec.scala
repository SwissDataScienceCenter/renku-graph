...
class ProjectDatasetsFinderSpec
  ...
  it should "return the very last modification of a dataset for the given project" in projectsDSConfig.use {
    implicit pcc =>
  ...
  it should "return non-modified datasets and the very last modifications of project's datasets" in projectsDSConfig
    .use { implicit pcc =>
  ...
  it should "return all datasets of the given project without merging datasets having the same sameAs" in projectsDSConfig
    .use { implicit pcc =>
  ...
  it should "return None if there are no datasets in the project" in projectsDSConfig.use { implicit pcc =>
    datasetsFinder.findProjectDatasets(Criteria(projectSlugs.generateOne)).asserting(_.results shouldBe List.empty)
  }

  it should "not returned deleted dataset" in projectsDSConfig.use { implicit pcc =>
  ...
  it should "not returned deleted dataset when its latest version was deleted" in projectsDSConfig.use { implicit pcc =>
  ...
  it should "return the results sorted by dateModified if requested" in projectsDSConfig.use { implicit pcc =>
  ...
  it should "return the results sorted by dateModified with a fallback to dateCreated or datePublished for non-modified DS" in projectsDSConfig
    .use { implicit pcc =>
  ...
  it should "return the results sorted by name by default and if requested" in projectsDSConfig.use { implicit pcc =>
  ...
  it should "return the requested page" in projectsDSConfig.use { implicit pcc =>
    val (dataset1 -> dataset2 -> modified2, project) = renkuProjectEntities(anyVisibility)
      .addDataset(datasetEntities(provenanceImportedExternal))
      .addDatasetAndModification(datasetEntities(provenanceInternal))
      .generateOne
  ...
  private def datasetsFinder(implicit pcc: ProjectsConnectionConfig) = new ProjectDatasetsFinderImpl[IO](tsClient)
}
