...
final case class DatasetViewedEvent(identifier: datasets.Identifier, dateViewed: datasets.DateViewed)

object DatasetViewedEvent {

  def forDataset(identifier: datasets.Identifier, now: () => Instant = () => Instant.now): DatasetViewedEvent =
    DatasetViewedEvent(identifier, dateViewed = datasets.DateViewed(now()))
  ...
  implicit val encoder: Encoder[DatasetViewedEvent] = Encoder.instance {
    case DatasetViewedEvent(identifier, dateViewed) =>
      json"""{
  ...
  implicit val decoder: Decoder[DatasetViewedEvent] = Decoder.instance { cursor =>
    ...
    } yield DatasetViewedEvent(identifier, date)
  }

  implicit val show: Show[DatasetViewedEvent] = Show.show { case DatasetViewedEvent(identifier, dateViewed) =>
