...
import io.renku.triplesgenerator.api.events.Generators.cleanUpEvents
import org.scalatest.EitherValues
import org.scalatest.matchers.should
import org.scalatest.wordspec.AnyWordSpec

class CleanUpEventSpec extends AnyWordSpec with should.Matchers with EitherValues {
  ...
  "json codec" should {
    ...
    "encode and decode" in {
      ...
      event.asJson.hcursor.as[CleanUpEvent].value shouldBe event
    }
    ...
    "fail if categoryName does not match" in {
      ...
      }""".hcursor.as[CleanUpEvent]

      result.left.value.getMessage() should include(s"Expected CLEAN_UP but got $otherCategory")
    }
  }
}
