...
class UpdatesCreatorSpec extends WordSpec with InMemoryRdfStore {
  ...
  "prepareUpdates" should {
    ...
    "generate a query inserting renku:topmostSameAs if not present yet" in new TestCase {
      ...
      (updatesCreator.prepareUpdates(TopmostData(entityId, sameAs, derivedFrom)) map (_.query) map runUpdate).sequence
        .unsafeRunSync()
    ...
    "generate a query updating renku:topmostSameAs if already present" in new TestCase {
      ...
      (updatesCreator.prepareUpdates(TopmostData(entityId, newTopmostSameAs, derivedFrom)) map (_.query) map runUpdate).sequence
        .unsafeRunSync()
    ...
    "generate a query updating renku:topmostDerivedFrom if already present" in new TestCase {
      ...
      (updatesCreator.prepareUpdates(TopmostData(entityId, sameAs, newTopmostDerivedFrom)) map (_.query) map runUpdate).sequence
        .unsafeRunSync()
