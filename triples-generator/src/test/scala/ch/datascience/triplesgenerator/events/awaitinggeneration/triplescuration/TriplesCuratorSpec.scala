...
import ch.datascience.triplesgenerator.events.awaitinggeneration.triplescuration.datasets.DataSetInfoEnricher
import ch.datascience.triplesgenerator.events.awaitinggeneration.triplescuration.forks.ForkInfoUpdater
  ...
  "curate" should {
    ...
    "pass the given triples through all the curation steps and return the final results" in new TestCase {
      ...
      (personDetailsUpdater.updatePersonDetails _)
        .expects(CuratedTriples[Try](triples, updatesGroups = Nil), event.project)
