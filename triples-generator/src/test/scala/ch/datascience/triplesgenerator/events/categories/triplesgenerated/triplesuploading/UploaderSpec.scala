...
import ch.datascience.triplesgenerator.events.categories.triplesgenerated.TransformationData.{ResultData, TransformationStep, TransformationStepResult}
...
class UploaderSpec extends AnyWordSpec with MockFactory with should.Matchers {
  ...
  "upload" should {
    ...
    s"return $DeliverySuccess if updates and triples uploading is successful" in new TestCase {
      ...
      val transformationData = transformationDataObjects[Try].generateOne.copy(steps =
        List(
          transformationSteps[Try].generateOne.copy(transformation = step1Transformation),
          transformationSteps[Try].generateOne.copy(transformation = step2Transformation)
        )
      )
      ...
      step1Transformation
        .expects(transformationData.projectMetadata)
      ...
      inSequence {
        ...
        (triplesUploader.upload _)
          .expects(transformationData.triples)
          .returning(DeliverySuccess.pure[Try])
      }

      uploader.upload(transformationData) shouldBe DeliverySuccess.pure[Try]
    }
    ...
    s"return $RecoverableFailure if running transformation step fails with a TransformationRecoverableError" in new TestCase {
      ...
      val transformationData = transformationDataObjects[Try].generateOne.copy(steps =
        List(
          TransformationStep[Try](step1Name, step1Transformation),
          transformationSteps[Try].generateOne
            .copy(transformation = mockFunction[ProjectMetadata, TransformationStepResult[Try]])
        )
      )
      ...
      step1Transformation
        .expects(transformationData.projectMetadata)
        .returning(EitherT.leftT[Try, ResultData[Try]](recoverableError))

      uploader.upload(transformationData) shouldBe RecoverableFailure(recoverableError.getMessage).pure[Try]
    }
    ...
    s"return $InvalidUpdatesFailure if a transformation step fails to run" in new TestCase {
      ...
      step1Transformation
        .expects(transformationData.projectMetadata)
        .returning(EitherT.right[ProcessingRecoverableError](exception.raiseError[Try, ResultData[Try]]))

      uploader.upload(transformationData) shouldBe InvalidUpdatesFailure(
    ...
    s"return $RecoverableFailure if executing transformation step queries fails with a RecoverableFailure" in new TestCase {
      ...
      uploader.upload(transformationData) shouldBe recoverableError.pure[Try]
    }
    ...
    s"return $InvalidUpdatesFailure if executing transformation step queries fails with a InvalidUpdatesFailure" in new TestCase {
      ...
      uploader.upload(transformationData) shouldBe nonRecoverableError.pure[Try]
    }
    ...
    Set(RecoverableFailure(nonEmptyStrings().generateOne),
      ...
      s"return ${failure.getClass.getSimpleName} if triples uploading failed with such failure" in new TestCase {
        ...
        val step1Transformation = mockFunction[ProjectMetadata, TransformationStepResult[Try]]
        val transformationData = transformationDataObjects[Try].generateOne.copy(
          steps = List(transformationSteps[Try].generateOne.copy(transformation = step1Transformation))
        )
        ...
        uploader.upload(transformationData) shouldBe failure.pure[Try]
      }
    }
  }
