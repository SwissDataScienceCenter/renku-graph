...
import GraphModelGenerators.{projectResourceIds, projectSlugs}
...
class AddProjectSlugSpec
  ...
  "migrate" should {
    ...
    "prepare backlog of projects requiring provisioning " +
      ...
      "pass the project to the Slug persister " +
      "and keep a note of the project once it's done" in {
        ...
        allProjectSlugs foreach { slug =>
          val projectInfo = ProjectInfo(projectResourceIds.generateOne, slug)
          givenProjectDataFetching(slug, returning = projectInfo.some.pure[IO])
          givenSlugPersisting(projectInfo, returning = ().pure[IO])
  ...
  private val slugPersister         = mock[SlugPersister[IO]]
  ...
  private lazy val migration = new AddProjectSlug[IO](
  ...
  private def givenSlugPersisting(projectInfo: ProjectInfo, returning: IO[Unit]) =
    (slugPersister.persistSlug _)
      .expects(projectInfo)
      .returning(returning)
