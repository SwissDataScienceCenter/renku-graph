  ...
  "fixDerivationHierarchies" should {
    ...
    "update topmostDerivedFrom on all derivation hierarchies " +
      "and remove existing wasDerivedFrom and topmostDerivedFrom" in new TestCase {
        ...
        def projectDataset(projectId: projects.ResourceId, expectedDS: entities.Dataset[entities.Dataset.Provenance]) =
          where { (projId: projects.ResourceId, ds: entities.Dataset[entities.Dataset.Provenance.Modified]) =>
            projId == projectId &&
        ...
        (updatesCreator
          .deleteOtherDerivedFrom(_: projects.ResourceId, _: entities.Dataset[entities.Dataset.Provenance.Modified]))
          .expects(projectDataset(finalProject.resourceId, mod1DS))
          .returning(mod1DSDerivedQueries)
        val mod1DSTopmostQueries = sparqlQueries.generateList()
        (updatesCreator
          .deleteOtherTopmostDerivedFrom(_: projects.ResourceId,
                                         _: entities.Dataset[entities.Dataset.Provenance.Modified]
          ))
          .expects(projectDataset(finalProject.resourceId, mod1DS))
        ...
        (updatesCreator
          .deleteOtherDerivedFrom(_: projects.ResourceId, _: entities.Dataset[entities.Dataset.Provenance.Modified]))
          .expects(projectDataset(finalProject.resourceId, mod2DS))
