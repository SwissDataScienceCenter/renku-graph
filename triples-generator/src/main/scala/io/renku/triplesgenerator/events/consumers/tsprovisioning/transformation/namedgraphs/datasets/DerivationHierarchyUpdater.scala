        ...
        projectAndQueriesF >>= { case (proj, quers) =>
          findTopmostDerivedFrom[F](originalDS, proj)
            ...
            .map { case (updatedProj, updatedDS) =>
              val derivedFromDeletions = deleteOtherDerivedFrom(project.resourceId, updatedDS)
              val topmostDeletions     = deleteOtherTopmostDerivedFrom(project.resourceId, updatedDS)
