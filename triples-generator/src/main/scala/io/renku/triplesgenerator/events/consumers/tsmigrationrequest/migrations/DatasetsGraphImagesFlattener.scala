...
private object DatasetsGraphImagesFlattener {

  private lazy val name = Migration.Name("Flatten Datasets graph images")
  ...
  private[migrations] lazy val query = SparqlQuery.of(
    ...
    sparql"""|DELETE {
             ...
             |    ?topSameAs renku:imagesConcat ?currentImgsConcat.
             |  }
             |}
             |INSERT {
             |  GRAPH ${GraphClass.Datasets.id} {
             |    ?topSameAs renku:imagesConcat ?imagesConcat.
             |  }
             |}
             |WHERE {
             |   SELECT ?topSameAs ?currentImgsConcat
             ...
             |     }
             |   }
             |   GROUP BY ?topSameAs ?currentImgsConcat
             |}
             |""".stripMargin
  )
}
