...
private class UpdatesCreator[Interpretation[_]: MonadThrow](gitLabApiUrl: GitLabApiUrl) {
  println(gitLabApiUrl)
  def prepareUpdates(person: Person): TransformationStep[Interpretation] = ???
//    TransformationStep[Interpretation](
//      name = "Persons details updates",
//      updates(person): _*
//    )
//
//  private def updates: Interpretation[Person => (ProjectMetadata, List[SparqlQuery])] = {
//    case Person(id, name, _, maybeEmail, _, maybeGitLabId) =>
//      List(
