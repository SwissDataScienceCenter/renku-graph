...
import monocle.function.Plated.transformM
...
private trait PersonExtractor[Interpretation[_]] {
  ...
  def extractPersons(triples: JsonLDTriples): Interpretation[(JsonLDTriples, Set[PersonData])]
}
...
private class PersonExtractorImpl[Interpretation[_]]()(implicit ME: MonadError[Interpretation, Throwable])
  ...
  override def extractPersons(triples: JsonLDTriples): Interpretation[(JsonLDTriples, Set[PersonData])] = {
    val persons = mutable.HashSet[PersonData]()
    for {
      updatedJson <- transformM(toJsonWithoutPersonDetails(persons))(triples.value)
    } yield JsonLDTriples(updatedJson) -> persons.toSet
  }

  private def toJsonWithoutPersonDetails(
      persons: mutable.Set[PersonData]
  )(json:      Json): Interpretation[Json] =
...
//  // TODO: pull this functionality out to PersonDetailsUpdater...needs to be refactored
...
//      case multipleNames =>
//        ME.raiseError {
//          new Exception("Multiple names for person in generated JSON-LD") // don't
//        }
//
//    }
//
