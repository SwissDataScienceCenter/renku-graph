...
private trait OrphanPartsRemover[Interpretation[_]] extends SingleQueryUpdater[Interpretation] {

  override val description = "Removing orphan Part entities"
  ...
  override val query = s"""|PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>
                           ...
                           |    FILTER NOT EXISTS { ?partResource schema:isPartOf ?project }
                           |  } 
                           |  {
                           |    ?partResource ?p ?o . 
                           |    BIND (?partResource as ?s)
                           |  } UNION {
                           |    ?s ?p ?partResource . 
                           |    BIND (?partResource as ?o)
                           |  }
                           |}
                           |""".stripMargin
}
