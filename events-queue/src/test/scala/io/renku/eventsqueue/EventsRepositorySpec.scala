...
class EventsRepositorySpec extends AsyncFlatSpec with AsyncIOSpec with EventsQueuePostgresSpec with should.Matchers {

  private val category  = categoryNames.generateOne
  private val chunkSize = 50

  it should "insert rows in status New into the DB so they are ready for processing" in testDBResource.use {
    implicit cfg =>
  ...
  it should "update rows in status New so they are not visible for processing" in testDBResource.use { implicit cfg =>
  ...
  it should "only delete rows in status PROCESSING" in testDBResource.use { implicit cfg =>
  ...
  it should "not pick events for processing " +
    "if they are in the Processing status for less than the grace period" in testDBResource.use { implicit cfg =>
  ...
  it should "be possible to change status of an event in Processing back to New" in testDBResource.use { implicit cfg =>
  ...
  private def findAllEvents(implicit cfg: DBConfig[TestDB]): IO[List[DequeuedEvent]] = {
  ...
  private def update(event: Json, status: EnqueueStatus, date: OffsetDateTime)(implicit cfg: DBConfig[TestDB]) =
    findInDB(event)
      .flatMap(e => updateEvent(e.id, status, date))

  private def findInDB(event: Json)(implicit cfg: DBConfig[TestDB]) =
    findAllEvents
      .map(_.find(_.payload == event.noSpaces).getOrElse(fail(s"no event with '${event.noSpaces}' payload")))

  private def updateEvent(id: Int, status: EnqueueStatus, updateDate: OffsetDateTime)(implicit
      cfg: DBConfig[TestDB]
  ): IO[Unit] = {
