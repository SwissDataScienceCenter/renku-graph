...
class ProjectAuthorizerImpl(
  ...
  private def validate(
    ...
    case ((Public, _) :: Nil, _)                                                             => EitherT.rightT(())
    case ((_, projectMembers) :: Nil, Some(authUser)) if projectMembers contains authUser.id => EitherT.rightT(())
    case _                                                                                   => EitherT.leftT(AuthorizationFailure)
  }
}
