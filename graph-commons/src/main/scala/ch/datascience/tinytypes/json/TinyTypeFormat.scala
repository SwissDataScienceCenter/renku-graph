...
class TinyTypeReads[V, T <: TinyType[V]](instantiate: V => T)(implicit jsonToV: JsValue => JsResult[V])
    extends Reads[T] {
...
object TinyTypeFormat {
  ...
  def apply[V, T <: TinyType[V]](
      valueToTinyType:    V => T
  )(implicit jsonToValue: JsValue => JsResult[V], valueToJson: V => JsValue): Format[T] =
    new Format[T] {
      override def writes(o: T): JsValue = TinyTypeWrites[V, T].writes(o)

      override def reads(json: JsValue): JsResult[T] = TinyTypeReads(valueToTinyType).reads(json)
    }
}
