...
class RateLimitSpec extends AnyWordSpec with ScalaCheckPropertyChecks with should.Matchers {
  ...
  "from" should {
    ...
    "return failure if 0" in {
      ...
      exception.getMessage shouldBe "io.renku.control.RateLimit has to be positive"
    }
    ...
    "return failure if negative number" in {
      ...
      exception            shouldBe an[IllegalArgumentException]
      exception.getMessage shouldBe "Invalid value for io.renku.control.RateLimit: '-1/sec'"
    }
    ...
    "return failure if not a number" in {
      ...
      exception            shouldBe an[IllegalArgumentException]
      exception.getMessage shouldBe "Invalid value for io.renku.control.RateLimit: 'add/sec'"
    }
