...
final case class ProcessRunActivity(override val id:              CommitId,
...
object ProcessRunActivity {
  ...
  implicit def encoder(implicit renkuBaseUrl: RenkuBaseUrl,
                       fusekiBaseUrl:         FusekiBaseUrl): JsonLDEncoder[ProcessRunActivity] =
    JsonLDEncoder.instance { entity =>
      JsonLD.entity(
        EntityId of (s"$fusekiBaseUrl/commit/${entity.id}${entity.maybeStepId.map(id => s"/$id").getOrElse("")}"),
